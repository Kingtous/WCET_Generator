...
import sys
import operator
def replace_call (basicblock_sum, funcs_sum, callfunc_sum, style):
    '''
    w：替换被call的部分
    b：记录task中call的函数名
            ...
            if basicblock_name!='return':
                ...
                while called_start_place!=-1:   #存在第二个label就是存在调用其他代码的部分
                    ...
                    if (basicblock_part[prestart_place:called_start_place].find("call") == -1):  #寻找"call"
                        ...
                        for i in range(called_start_place,len(basicblock_part)):
                            if basicblock_part[i]=='{':
                                ...
                                if brackets_num==0:
                                    ...
                                    break
                        # try:
                        basicblock_sum[basicblock_name]= basicblock_sum[basicblock_name][:replace_begin_place] + replace_part + basicblock_sum[basicblock_name][replace_end_place:]
                        ...
                        replace_string=replace_string[:call_start_palce+1]+"callfunction"+replace_string[call_end_palce:]
                        basicblock_sum[basicblock_name]= basicblock_sum[basicblock_name][:replace_begin_place] + replace_string + basicblock_sum[basicblock_name][replace_end_place:]
                        called_start_place=basicblock_sum[basicblock_name].find(flag, replace_end_place)
...
def findcallname(func_part, called_func, Reference, funcs_sum, callfunc_sum):
    ...
    while callfunc_name_startplace != -1:
        if (func_part[callfunc_name_startplace - 1] == ' ' or func_part[callfunc_name_startplace - 1] == '{') and (func_part[callfunc_name_startplace + 4] == ' ' or func_part[callfunc_name_startplace + 4] == '{'):
            ...
            for i in range(0, flag_startplace):
                if func_part[flag_startplace - 1 - i] == '"':
                    ...
                    break
            a= func_part[flag_endplace + 1:flag_startplace]
            callfunc_sum[a] = func_part[callfunc_name_startplace + 1:callfunc_name_endplace]
            if (func_part[callfunc_name_startplace + 1:callfunc_name_endplace] not in temp_Reference) and(func_part[callfunc_name_startplace + 1:callfunc_name_endplace] in funcs_sum.keys()):
                called_func[a]= func_part[callfunc_name_startplace + 1:callfunc_name_endplace]
                temp_Reference.append(func_part[callfunc_name_startplace + 1:callfunc_name_endplace])
            # called_func.append(func_part[call_name_st + 1:call_name_en])
